...
extension RaceStore {
 func previewCircuit(_ circuit:SeasonCircuit) {
...
struct CircuitLibraryView:View {
 ...
 var body:some View {
  VStack(alignment:.leading,spacing:16) {
   ...
   let upcoming=CircuitCatalog.upcoming()
   ForEach(upcoming) {c in circuitRow(c,next:c.id==upcoming.first?.id)}
   if showPast {ForEach(CircuitCatalog.all.filter{$0.raceDate<Date()}) {c in circuitRow(c,next:false)}}
