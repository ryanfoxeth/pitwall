...
@MainActor func validateSeasonScenery(_ race:RaceStore) async {
 ...
 for id in CircuitScenery.circuits {
  ...
  for theme in RaceTheme.allCases {
   ...
   if !loaded {failures.append("Missing \(id) \(theme.rawValue)")}
   ...
   for width:Float in [0.7,2.0] {
    ...
    let good=(0..<3).allSatisfy{bounds.min[$0]>=available.min[$0] - 0.002 && bounds.max[$0]<=available.max[$0] + 0.002 && bounds.extents[$0].isFinite}
